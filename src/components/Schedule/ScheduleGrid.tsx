...
export function ScheduleGrid({
  ...
  const nonEmptySlots = slots.filter((slot) => {
      ...
      <table className="border-collapse w-full min-w-max">
        <thead>
          <tr className="bg-[#1a1d27]">
            <th className="md:sticky left-0 z-10 bg-[#1a1d27] px-3 py-3 text-left text-xs font-semibold text-[#94a3b8] uppercase tracking-wider border-b border-r border-[#2e3347] min-w-[80px]">
              Hora
            </th>
            {columns.map((col) => (
              <th
        ...
        <tbody>
          {nonEmptySlots.map((slot, i) => {
            ...
            return (
              ...
              >
                <td className="md:sticky left-0 z-10 px-3 py-2 text-xs text-[#94a3b8] font-mono border-r border-b border-[#2e3347] align-top whitespace-nowrap bg-inherit">
                ...
                {columns.map((col) => {
                  const cell = rowHorarios.filter(
                    (h) => getColumnId(h) === col.id,
                  );
                  return (
                    <td
                      key={col.id}
