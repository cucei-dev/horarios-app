...
function App() {
  const { data: calendarios = [], isLoading: loadingCals } = useCalendarios();
  const { data: centros = [], isLoading: loadingCentros } = useCentros();

  const [selectedCalendario, setSelectedCalendario] = useState<number | null>(
    null,
  );
  ...
  const [aulasState, setAulasState] = useState<{
    aulas: Aula[];
  ...
  useEffect(() => {
    ...
    fetchAulas(selectedEdificio)
      ...
      .catch((e) => {
        console.error(e);
        setAulasState({ aulas: [], loadedFor: selectedEdificio });
      });
  }, [selectedEdificio]);

  const currentAulas = selectedEdificio ? aulasState.aulas : [];
  const loadingAulas = !!(
    selectedEdificio && aulasState.loadedFor !== selectedEdificio
  );
        ...
        <div className="max-w-screen-2xl mx-auto flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#94a3b8]">
              Consulta la disponibilidad de aulas
            </p>
          </div>
        </div>
      </header>
          ...
          {!selectedEdificio ? (
            <div className="flex flex-col items-center justify-center h-64 gap-3 text-center">
              ...
              <p className="text-[#94a3b8] text-sm">
                Selecciona un calendario, centro universitario y edificio para
                ver los horarios.
              </p>
            </div>
          ) : loadingAulas ? (
